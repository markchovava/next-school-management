...
import { formGradeDeleteApiAction, formGradeListApiAction, formGradePaginationApiAction, formGradeSearchApiAction } from '@/actions/formGradeActions';
import Loader from '@/components/Loader';



export default function FormGradeList({ dbData }) {
  const [isModal, setIsModal] = useState(false);
  const { formGradeState, formGradeDispatch} = AdminContextState();
  const [isPending, startTransition] = useTransition();
  const [search, setSearch] = useState('');

  useEffect(() => {
    formGradeDispatch({type: 'ADD_DATA', payload: {
        items: dbData?.data,
    ...
    async function paginateHandler(url) {
        try{
            const res = await formGradePaginationApiAction(url)
            formGradeDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        if(!search) {
          ...
          try{
            const res = await formGradeSearchApiAction(search);
            startTransition(() => res);
            formGradeDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
    ...
    async function getData() {
        try{
            const res = await formGradeListApiAction();
            formGradeDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await formGradeDeleteApiAction(id);
            if(res.status == 1) {
                await getData();
    ...
    if(!formGradeState.items) { return <Loader /> }
  ...
  return (
    <>
      ...
      <section className='mx-8 pt-8 pb-12 flex items-center justify-end gap-3'>
          {/* PREVIOUS */}
          {formGradeState.prevURL &&
          <button
              title={formGradeState.prevURL}
              onClick={() => paginateHandler(formGradeState?.prevURL)}
          ...
          {/* NEXT */}
          {formGradeState.nextURL &&
          <button 
              title={formGradeState.nextURL}
              onClick={() => paginateHandler(formGradeState?.nextURL)}
              className='group flex items-center justify-center gap-2 text-cyan-700 '>
              <span>Next</span>
