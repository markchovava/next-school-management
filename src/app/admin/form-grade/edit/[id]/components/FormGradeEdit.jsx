"use client";
import { formGradeUpdateApiAction } from '@/actions/formGradeActions';
...
export default function FormGradeEdit({ dbData, id }) {
  ...
  async function postData() {
    ...
    try{
        const res = await formGradeUpdateApiAction(formData, id);
        startTransition(() => res)
        if(res.status == 1) {
            toast.success(res.message, reactToastifyDark);
            setErrMsg({});
            router.push(`/admin/form-grade/${id}`);
            return;
        }
        setErrMsg({});
        } catch (error) {
