...
import { subjectDeleteApiAction, subjectListApiAction, subjectPaginationApiAction, subjectSearchApiAction } from '@/actions/subjectActions';
import Loader from '@/components/Loader';



export default function SubjectList({ dbData }) {
  console.log(dbData)
  const [isModal, setIsModal] = useState(false);
  const { subjectState, subjectDispatch} = AdminContextState();
  const [isPending, startTransition] = useTransition();
  const [search, setSearch] = useState('');

  useEffect(() => {
    subjectDispatch({type: 'ADD_DATA', payload: {
        items: dbData?.data,
    ...
    async function paginateHandler(url) {
        try{
            const res = await subjectPaginationApiAction(url)
            subjectDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        if(!search) {
          ...
          try{
            const res = await subjectSearchApiAction(search);
            startTransition(() => res);
            subjectDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
    ...
    async function getData() {
        try{
            const res = await subjectListApiAction();
            subjectDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await subjectDeleteApiAction(id);
            if(res.status == 1) {
                await getData();
    ...
    console.log('subjectState.items')
    console.log(subjectState.items)

    if(!subjectState.items) { return <Loader /> }
    ...
    <>
      ...
      <section className="w-[100%] lg:overflow-hidden overflow-scroll">
        <section className='lg:w-[100%] w-[60rem]'>
          ...
          {/* DETAILS */}
          { subjectState?.items?.length > 0 ?
            subjectState.items.map((i, key) => (
      ...
      <section className='mx-8 pt-8 pb-12 flex items-center justify-end gap-3'>
          {/* PREVIOUS */}
          {subjectState.prevURL &&
          <button
              title={subjectState.prevURL}
              onClick={() => paginateHandler(subjectState?.prevURL)}
          ...
          {/* NEXT */}
          {subjectState.nextURL &&
          <button 
              title={subjectState.nextURL}
              onClick={() => paginateHandler(subjectState?.nextURL)}
              className='group flex items-center justify-center gap-2 text-cyan-700 '>
              <span>Next</span>
