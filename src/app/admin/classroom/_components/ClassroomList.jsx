...
import { classroomDeleteApiAction, classroomListApiAction, classroomPaginationApiAction, classroomSearchApiAction } from '@/actions/classroomActions';
import Loader from '@/components/Loader';



export default function ClassroomList({ dbData }) {
    const [isModal, setIsModal] = useState(false);
    const { classroomState, classroomDispatch} = AdminContextState();
    const [isPending, startTransition] = useTransition();
    const [search, setSearch] = useState('');

    useEffect(() => {
      classroomDispatch({type: 'ADD_DATA', payload: {
          items: dbData?.data,
    ...
    async function paginateHandler(url) {
        try{
            const res = await classroomPaginationApiAction(url)
            classroomDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        if(!search) {
          ...
          try{
            const res = await classroomSearchApiAction(search);
            startTransition(() => res);
            classroomDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
    ...
    async function getData() {
        try{
            const res = await classroomListApiAction();
            classroomDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await classroomDeleteApiAction(id);
            if(res.status == 1) {
                await getData();
    ...
    if(!classroomState.items) { return <Loader /> }
    ...
    <>
      ...
      <section className="w-[100%] lg:overflow-hidden overflow-scroll">
        <section className='lg:w-[100%] w-[60rem]'>
          ...
          {/* DETAILS */}
          { classroomState?.items?.length > 0 ?
            classroomState.items.map((i, key) => (
            <div key={key} className='mx-auto w-[96%] bg-white py-2 flex items-center justify-start border border-slate-300 '>
              ...
              <div className='w-[15%] px-3 py-1 text-xl flex items-center justify-end gap-2'>
                <Link title='View' href={`/admin/classroom/${i.id}`}> 
                  <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> 
                </Link>
                <Link title='Edit' href={`/admin/classroom/edit/${i.id}`}> 
      ...
      <section className='mx-8 pt-8 pb-12 flex items-center justify-end gap-3'>
          {/* PREVIOUS */}
          {classroomState.prevURL &&
          <button
              title={classroomState.prevURL}
              onClick={() => paginateHandler(classroomState?.prevURL)}
          ...
          {/* NEXT */}
          {classroomState.nextURL &&
          <button 
              title={classroomState.nextURL}
              onClick={() => paginateHandler(classroomState?.nextURL)}
              className='group flex items-center justify-center gap-2 text-cyan-700 '>
              <span>Next</span>
