"use client";
import { classroomStoreApiAction } from '@/actions/classroomActions';
...
export default function ModalAdd({isModal, setIsModal, getData}) {
    ...
    async function postData() {
        ...
        try{
            const res = await classroomStoreApiAction(formData);
            startTransition(() => res)
            if(res.status == 1) {
  ...
  return (
    ...
    <AnimatePresence>
        ...
        <section className='mx-auto w-[50%] bg-white text-black p-6 rounded-2xl'>
            ...
            <h3 className='text-[2rem] font-light text-center mb-3'>Add Classroom</h3>
